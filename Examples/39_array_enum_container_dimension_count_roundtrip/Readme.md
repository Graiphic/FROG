# Example 39 - Enum Array Dimension Count Property Roundtrip
...
- Each visible cell references the Default Enum realization through `element.template_ref` and `element.asset_ref`.
- Runtime overlays must align to Default Array `index_display`, `element_region`, `element_slot`, and scrollbar parts. Enum cells consume the child widget's `placement_bounds` aura as the Array cell footprint, exactly like the Default Numeric Array path. `value_face`, `spinner`, `increment_up`, and `increment_down` remain internal Enum widget parts and must not be used as the cell boundary. No selector-face or selector-arrow part is exposed.
- The Default Enum compact Array footprint is the non-rendered `placement_bounds` rectangle (`176 x 38` source units) with a uniform 4-unit aura band around the visible body. Control and indicator variants share the same cell footprint; the indicator changes visual fill, not layout law.
- Enum increment/decrement hover and pressed states must survive Array containment because the buttons remain the child widget's semantic `increment_up` / `increment_down` parts. The reviewed Default posture also keeps `data_entry.increment_wrap=true`, so contained Enum stepping wraps circularly through enabled items.
- The opened Enum item list is an Array-host overlay surface. It must render above the Array viewport and cells, use the dropdown SVG skin, and must not be clipped by the repeated cell box.
- No local Example SVG skin is duplicated.
- Runtime-family validation for this post-boundary example must preserve the same source-owned N-dimensional rank, index-display wrapping, final-two-dimensions projection, visible row/column command semantics, and Enum cell roundtrip across the private hosts that implement this slice.
