# Example 34 Visual Contract

Example 34 records the accepted browser-host reference surface for
`Array 2D Visible Counts Property Roundtrip`.

## Accepted Visible Surface

- `visible_row_count`: Numeric U8 control that requests the Array visible row count.
- `visible_column_count`: Numeric U8 control that requests the Array visible column count.
...
- The visible-row and visible-column Numeric controls can be edited without
  changing the Array viewport.
- `Execute` consumes the Numeric U8 values and writes
  `numeric_array.viewport.visible_counts[0]`,
  `numeric_array.viewport.visible_counts[1]`,
  `published_array.viewport.visible_counts[0]`, and
