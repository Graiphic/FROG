...
The accepted control posture also exposes the LabVIEW-like increment/decrement command as semantic skin parts: `spinner`, `increment_up`, and `increment_down`. `display.increment_buttons_visible` controls whether that command is shown, and `data_entry.increment_wrap` controls whether stepping past the first or last declared item wraps or clamps.
