; Example 19 Subpanel Embedded Front Panel native proof kernel.
; The Subpanel widget hosts another .frog front panel; this kernel only
; publishes a manifest-backed scene_ready bool so the example remains on the
; native artifact corridor without assigning scalar value semantics to the
; Subpanel support widget.

%FrogBoolRunResult = type { i8, i8, i16 }

define dso_local void @frog_example19_run(i8 %scene_ready_request, ptr %out_result) {
entry:
