; Example 22 Listbox Selection Roundtrip native proof kernel.
; The selected Listbox numeric item value is copied to the public output so the
; example remains on the manifest-backed native artifact corridor.

%FrogRunResult = type { i8, i16, i16 }

define dso_local void @frog_example22_run(i16 %item_value, ptr %out_result) {
entry:
  ...
  store i16 %item_value, ptr %result_ptr, align 2
