...
std::shared_ptr<const frog::runtime::NativeBoolKernelBridge> make_bridge() {
    return std::make_shared<const frog::runtime::NativeBoolKernelBridge>(
        frog::runtime::make_linked_native_bool_kernel_bridge(native_manifest_path(), &frog_example10_run));
}
...
void test_direct_llvm_button_kernel_bridge_call() {
    const auto bridge = make_bridge();
    assert(bridge->manifest().entry_symbol == "frog_example10_run");
    assert(bridge->manifest().abi == "frog_bool_to_result_status_outptr");
    assert(bridge->manifest().source_artifact == "Implementations/Reference/LLVM/examples/10_button_press_to_boolean/kernel.ll");
    assert(bridge->manifest().source_lowered_unit == "Examples/10_button_press_to_boolean/main.lowering.json");
    assert(bridge->manifest().input_id == "trigger_pressed");
    assert(bridge->manifest().output_id == "pressed");
...
void test_runtime_uses_llvm_button_kernel_bridge() {
...
void test_browser_ui_runtime_uses_llvm_button_kernel_bridge() {
...
int main() {
    test_direct_llvm_button_kernel_bridge_call();
    test_runtime_uses_llvm_button_kernel_bridge();
    test_browser_ui_runtime_uses_llvm_button_kernel_bridge();
    std::cout << "slice10 LLVM-produced native Button bool kernel bridge passed" << std::endl;
    return 0;
}
