...
std::shared_ptr<const frog::runtime::NativeStringKernelBridge> make_bridge() {
    return std::make_shared<const frog::runtime::NativeStringKernelBridge>(
        frog::runtime::make_linked_native_string_kernel_bridge(native_manifest_path(), &frog_example07_run));
}
...
void test_direct_llvm_string_kernel_bridge_call() {
    const auto bridge = make_bridge();
    assert(bridge->manifest().entry_symbol == "frog_example07_run");
    assert(bridge->manifest().abi == "frog_string_utf8_256_to_result_status_outptr");
    assert(bridge->manifest().source_artifact == "Implementations/Reference/LLVM/examples/07_string_value_roundtrip/kernel.ll");

    const auto result = bridge->run("hello world");
    assert(result.ok);
    assert(result.result == "hello world");
    assert(result.error_code == 0);
}

void test_runtime_uses_llvm_string_kernel_bridge() {
    frog::runtime::Slice07StringRuntimeCore runtime(contract_path(), wfrog_path());
    const auto artifact = runtime.execute_with_native_kernel_bridge(*make_bridge(), "hello world");
    assert(artifact.as_object().at("outputs").as_object().at("public").as_object().at("result_text").as_string() == "hello world");
    assert(artifact.as_object().at("outputs").as_object().at("ui").as_object().at("str_result").as_string() == "hello world");
}

void test_browser_ui_runtime_uses_llvm_string_kernel_bridge() {
    frog::runtime::StringBrowserUiRuntime runtime(contract_path(), wfrog_path(), make_bridge());
    const auto artifact = runtime.run_once("hello world");
    assert(artifact.as_object().at("outputs").as_object().at("public").as_object().at("result_text").as_string() == "hello world");
    ...
    assert_contains(html, "Example 07");
    assert_contains(html, "C++ reference runtime");
    assert_contains(html, "native kernel bridge");
    assert_contains(html, "LLVM native string kernel artifact");
...
int main() {
    test_direct_llvm_string_kernel_bridge_call();
    test_runtime_uses_llvm_string_kernel_bridge();
    test_browser_ui_runtime_uses_llvm_string_kernel_bridge();
    std::cout << "slice07 LLVM-produced native string kernel bridge passed" << std::endl;
    return 0;
}
