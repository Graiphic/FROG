...
bool is_button_switch_when_released_example(const std::string& example_id) {
...
bool is_switch_when_released_action(const Object& properties) {
    return json_string(properties, "behavior.mechanical_action") == "switch_when_released";
}

bool is_stored_switch_action(const Object& properties) {
...
Value Slice08EnumRuntimeCore::execution_artifact() const {
    ...
    for (const auto& entry : widgets) {
        ...
        copy_property("style.popup.fill_color");
        copy_property("style.popup.border_color");
        copy_property("style.popup.border_width");
        copy_property("style.popup.border_width_mode");
        copy_property("style.popup.option.fill_color");
        copy_property("style.popup.option.text_color");
        copy_property("style.popup.option.hover_fill_color");
        copy_property("style.popup.option.hover_text_color");
        copy_property("style.popup.option.selected_fill_color");
        copy_property("style.popup.option.selected_text_color");
        copy_property("style.popup.option.font_family");
        copy_property("style.popup.option.font_size");
        copy_property("style.popup.option.font_size_mode");
        copy_property("style.popup.option.font_weight");
        copy_property("style.popup.option.font_style");
        copy_property("style.popup.option.padding_inline");
        copy_property("style.popup.option.padding_inline_mode");
        copy_property("style.popup.option.height");
        copy_property("style.popup.option.height_mode");
        copy_property("interaction.enabled");
        copy_property("interaction.read_only");
...
Value Slice10ButtonRuntimeCore::execution_artifact() const {
    ...
    const auto execution_mode = button_execution_mode(button.properties);
    for (const auto& entry : widgets) {
        const auto& widget = entry.second;
        const bool value = json_bool(widget.properties, "value", false);
