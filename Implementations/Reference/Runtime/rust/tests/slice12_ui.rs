...
fn wfrog_path() -> PathBuf {
    repo_root().join("Examples/12_button_switch_when_released/ui/button_panel.wfrog")
}

#[test]
fn rust_example12_headless_button_switch_when_released() {
    ...
    assert_eq!(artifact["contract_ref"]["source_ref"]["example_id"].as_str(), Some("12_button_switch_when_released"));
    ...
    assert_eq!(artifact["execution_summary"]["mode"].as_str(), Some("button_switch_when_released"));
    assert_eq!(artifact["execution_summary"]["trigger_pressed"].as_bool(), Some(true));
    assert_eq!(artifact["outputs"]["public"]["switched"].as_bool(), Some(false));
    assert_eq!(artifact["outputs"]["ui"]["trigger_button"].as_bool(), Some(false));
    assert_eq!(artifact["outputs"]["ui"]["switched_indicator"].as_bool(), Some(false));

    let artifact = runtime.run_once(false).expect("switch true on release");
    ...
    let artifact = runtime.run_once(false).expect("switch false on release");
    assert_eq!(artifact["outputs"]["public"]["switched"].as_bool(), Some(false));
    assert_eq!(artifact["outputs"]["ui"]["trigger_button"].as_bool(), Some(false));
    assert_eq!(artifact["outputs"]["ui"]["switched_indicator"].as_bool(), Some(false));
}
...
#[test]
fn rust_example12_browser_ui_consumes_default_svg_and_source_styles() {
    let mut runtime = ButtonBrowserUiRuntime::with_native_kernel_bridge(contract_path(), wfrog_path(), None).expect("runtime");
    let html = runtime.render_html();

    assert!(html.contains("Button Switch When Released"));
    assert!(html.contains("Example 12 - .frog switch_when_released Button value"));
    assert!(html.contains("Rust reference runtime"));
    assert!(html.contains("button switch contract executor"));
    assert!(html.contains("data-execution-path='rust_button_switch_when_released_contract_executor'"));
    ...
    assert!(html.contains("data-frog-mechanical-action='switch_when_released'"));
    ...
    assert!(html.contains("mechanicalAction !== \"switch_when_released\""));
    assert!(html.contains("releaseIfArmed"));
