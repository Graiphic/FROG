...
<table>
  ...
  <tbody>
    ...
    <tr><td><a href="./Image.md"><code>Image.md</code></a></td><td>Image Static</td><td><code>frog.widgets.image_static</code></td><td>support media</td><td>support</td><td>static pasted/imported media baseline</td></tr>
...
<ul>
  ...
  <li>Placement bounds are the widget aura used by IDEs, front panels, and container widgets. All widget families use the same vocabulary: <code>placement_bounds</code> for the aura, <code>main_body</code> for the primary visible body, <code>aura_band</code> for the spacing between them, and <code>focus_ring</code> for focus state. Widget-specific SVG part names such as <code>value_face</code>, <code>state_face</code>, or <code>face</code> are mappings to that vocabulary, not separate placement rules. The adopted default <code>aura_band</code> is uniform <code>4px</code> at 1:1 scale unless a widget class explicitly declares another band. Placement bounds snap to the common grid defined in <a href="./PlacementGrid.md"><code>PlacementGrid.md</code></a> only when source or container policy declares snap.</li>
