...
def load_acceptance_support(acceptance: dict[str, object]) -> dict[str, object]:
    refs = acceptance.get("artifact_refs")
    support: dict[str, object] = {}
    if isinstance(refs, dict) and isinstance(refs.get("wfrog_path"), str):
        support["wfrog"] = load_json(ROOT / refs["wfrog_path"])
    return support
...
def test_example05_generic_contract_executor_matches_published_snapshot_and_overflow() -> None:
    acceptance = load_json(ROOT / "Implementations/Reference/Runtime/acceptance/example05_runtime_family.acceptance.json")
    refs = acceptance["artifact_refs"]
    contract = load_json(ROOT / refs["contract_path"])
    snapshot = load_json(ROOT / refs["snapshot_path"])
    support = load_acceptance_support(acceptance)

    observed = execute_acceptance(acceptance, contract, support)

    assert observed == snapshot
    check_acceptance_against_snapshot(acceptance, contract, snapshot, support)
    check_overflow_against_acceptance(acceptance, contract, support)


def test_example05_execute_reference_contract_cli_check_passes() -> None:
    result = subprocess.run(
        [
            sys.executable,
            "Implementations/Reference/Runtime/execute_reference_contract.py",
    ...
    assert result.returncode == 0, result.stdout + result.stderr
    assert "Reference contract execution check ok: 05_bounded_ui_accumulator" in result.stdout
