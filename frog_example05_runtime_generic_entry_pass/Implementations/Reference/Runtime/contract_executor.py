...
This module interprets published backend contracts through their contract shape
and unit kind rather than through hard-coded example identifiers.
...
- bounded_executable_ui_unit

The bounded executable UI unit is the first partial entry of Example 05 into the
generic executor. The specialized Example 05 checker is intentionally kept in
parallel for richer runtime-family validation.
"""
...
def case_for_bounded_ui_acceptance(acceptance: dict[str, Any]) -> dict[str, Any]:
    ...
    if isinstance(headless, dict) and isinstance(headless.get("input_value"), int):
        ...
        return {
            ...
            "widget_values": {"ctrl_input": value},
        }
...
def property_write_value(write: dict[str, Any]) -> Any:
...
def execute_bounded_executable_ui_unit(contract: dict[str, Any], unit: dict[str, Any], case: dict[str, Any], support_artifacts: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if body_rule.get("expression") != "state_next = state_current + input_value":
        raise ContractExecutionError("unsupported bounded executable body expression")
    ...
    property_writes = require_list(unit.get("property_writes"), "unit.property_writes")
    applied_widget_references: list[dict[str, Any]] = []
    property_map: dict[tuple[str, str], Any] = {}
    for write in property_writes:
        obj = require_object(write, "unit.property_writes[]")
        if obj.get("operation") != "frog.ui.property_write":
            raise ContractExecutionError("bounded_executable_ui_unit supports only frog.ui.property_write property_writes")
        widget_id = obj.get("widget_id")
        member = obj.get("member")
        if not isinstance(widget_id, str) or not isinstance(member, str):
            raise ContractExecutionError("property writes require widget_id and member strings")
    ...
    return {
        ...
        "outputs": {
            ...
            "ui": {
                "ctrl_input": input_value_raw,
                "ind_result": final_state,
            },
        },
        "ui_runtime": {
            "panel": {
...
def check_overflow_against_acceptance(acceptance: dict[str, Any], contract: dict[str, Any], support_artifacts: dict[str, Any] | None = None) -> None:
    ...
    try:
        execute_contract_case(
            contract,
            {
                "inputs": {"input_value": input_value},
                "widget_values": {"ctrl_input": input_value},
            },
            support_artifacts,
        )
